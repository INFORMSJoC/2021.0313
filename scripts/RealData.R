...
for(i in 1:length(df)){
  par(mar=c(3,4,3,2))
  plot(as.numeric(y), type='p', col='grey80', pch=19, cex= 0.6, xaxt="n",
       main=paste0("df = ", df[i]),
  ...
  lines(resL1$beta[, idx1], col = "blue", type='l', lty=2, lwd=2) ## plot the L1-TF estimate for a given df
}
par(op)
dev.off()
...
for(i in 1:length(df_zoom)){
  par(mar=c(3,4,3,2))
  plot(as.numeric(y)[idx.zoom], type='p', col='grey80', pch=19, cex= 0.6, xaxt="n",
  ...
  idx1 <- which(resL1$df==df_zoom[i])[1] # Choosing the one with the largest lambda value
  lines(resL1$beta[idx.zoom, idx1], col = "blue", type='l', lty=2, lwd=2) ## plot the L1-TF estimate for a given df

  rect(6, -10, 16, 120, density = 30, border = "transparent", angle = -30, col="black") # HATO
  rect(96, -10, 104, 120, density = 30, border = "transparent", angle = -30, col = "black") # PAKHAR
