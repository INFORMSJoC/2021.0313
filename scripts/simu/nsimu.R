...
for (n in nn){
  ...
  TabL0 = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunL0TF(dgm = "Blocks", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabL1 = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunL1TF(dgm = "Blocks", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabSp = foreach(mcseed = 1:iter2, .combine = rbind) %dopar%
    SingleRunSpline(dgm = "Blocks", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabPt = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunPeltTF(dgm = "Blocks", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabWs = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunwbsTF(dgm = "Blocks", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabNt = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunNotTF(dgm = "Blocks", n=n, sigma=sigma, seed=mcseed, dist="normal")
  if (n<=224){
...
for (n in nn){
  cat("\nRunning n =", n)
  TabL0 = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunL0TF(dgm = "Wave", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabL1 = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunL1TF(dgm = "Wave", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabSp = foreach(mcseed = 1:iter2, .combine = rbind) %dopar%
    SingleRunSpline(dgm = "Wave", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabCp = foreach(mcseed = 1:iter, .combine = rbind) %dopar% 
    SingleRunCpopTF(dgm = "Wave", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabNt = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunNotTF(dgm = "Wave", n=n, sigma=sigma, seed=mcseed, dist="normal")
  if (n<128){
    TabLc = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
...
for (n in nn){
  cat("\nRunning n =", n)
  TabL0 = foreach(mcseed = 1:iter, .combine = rbind) %dopar% 
    SingleRunL0TF(dgm = "Doppler", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabL1 = foreach(mcseed = 1:iter, .combine = rbind) %dopar% 
    SingleRunL1TF(dgm = "Doppler", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabSp = foreach(mcseed = 1:iter2, .combine = rbind) %dopar% 
    SingleRunSpline(dgm = "Doppler", n=n, sigma=sigma, seed=mcseed, dist="normal")
  TabNt = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
    SingleRunNotTF(dgm = "Doppler", n=n, sigma=sigma, seed=mcseed, dist="normal")
  if (n<96){
    TabLc = foreach(mcseed = 1:iter, .combine = rbind) %dopar%
