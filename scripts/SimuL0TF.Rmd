...
RunPlot = function(data, resL0, resL1, q){
  ltype = ifelse(q==0, "s", "l")
  plot(data$x, data$y, type='p', col='grey50', xlab="", ylab="", cex=1.2)
  lines(data$x, data$y0, col="black", lty=2, lwd=2, type=ltype) 
  lines(data$x, resL0$alpha, col="red", lwd=2, type=ltype)
  lines(data$x, resL1$beta, col="blue", lwd=2, lty = 6, type=ltype)
  points(data$x[resL0$A], rep(par("usr")[4], length(resL0$A)), pch=3, cex=2, col=2, lwd=2)
  legend("bottomright", c("True", "L0-TF", "L1-TF"),
         lty=c(2,1,6), col=c("black", "red", "blue"), lwd=2,
         cex=1.2, bty="n")
}

# png("../L0TF_IJOC_Revision/l0tf0513/ToyExample2.png", pointsize = 8, width=850, height=400, res = 120)  
png("ToyExample2.png", pointsize = 8, width=850, height=400, res = 120) 
par(mfrow=c(1,2),mar=c(3,3,3,3))
...
ltype = ifelse(q==0, "s", "l")
plot(data$x, data$y, type='p', col='grey50', xlab="", ylab="", cex=1.2)
lines(data$x, data$y0, col="black", lty=2, lwd=3, type=ltype) 
lines(data$x, resL0$alpha, col="red", lwd=2, type=ltype)
lines(data$x, resL1$beta, col="blue", lwd=2, lty=6, type=ltype)
points(data$x[resL0$A], rep(par("usr")[4], length(resL0$A)), pch=3, cex=2, col=2, lwd=2)
legend("bottom", c("True", "L0-TF", "L1-TF"), lty=c(2,1,6), col=c("black", "red", "blue"), lwd=c(2,2,2),
       horiz=T,  bty="n",
       cex=1.2)
...
## Figure 7: L0-TF estimates for nknot=5 and nknot=9
```{r fig.align="center", fig.width=10, fig.asp=0.5}

# png("../L0TF_IJOC_Revision/l0tf0513/
png("SimuEx1_2.png", pointsize = 8, width=850, height=400, res = 120)
n=300; sigma=0.1; q=1; nknot=5; seed=1
data = SimuEx(n=n, sigma=sigma, q=q, nknot=nknot, seed=seed)
sighat = median(abs(diff(data$y, diff = q+1)))/(qnorm(3/4)*sqrt(choose(2*(q+1), q+1)))
...
resL0 = samias(as.numeric(data$y), D_type= "tfq", q=q, kmax=nknot+4, eps=(0.96*sighat)^2, adjust = TRUE)
ltype = ifelse(q==0, "s", "l")
plot(data$x, data$y, type='p', col='grey80', xlab="", ylab="", cex=1.2)
lines(data$x, resL0$alpha.all[,5], col="black", lty=3, lwd=3, type=ltype) 
lines(data$x, resL0$alpha.all[,9], col="red", lty=2, lwd=2, type=ltype)

legend("bottom", c("k = 5", "k = 9"), lty=c(3,2), col=c("black", "red"), lwd=c(2,2),
